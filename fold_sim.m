function finResults = fold_sim(t0, dt, tmax, numSims)
	% Model parameters
	...
	x0 = sqrt(-bl); 	 % intial condition (equilibrium value)
	...
	for j = 1:1:numSims
		...
		for i = 1:1:(fix(tburn./dt))
			x0 = x0 + nform_fold(x0,bl).*dt + dW_x_burn(i,1);
		end	
		...
		for i =1:1:(length(t)-1)
			x(i+1,1) = x(i,1) + nform_fold( x(i,1), b(i,1) ).*dt + dW_x(i,1);
		end
